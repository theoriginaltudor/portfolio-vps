...
export async function logoutUser(pathname: string) {
  const { error, response } = await authApiCall("/api/Login/logout", { method: "POST" });
  ...
  if (response) {
    ...
    if (expiredAuth) {
      ...
      cookieStore.set("auth", valueMatch ? valueMatch[1] : "", {
        ...
        domain: domainMatch ? domainMatch[1] : (process.env.NODE_ENV === "production" ? ".tudor-dev.com" : undefined),
